...
void generer_map(SDL_Renderer* renderer, int h, int w, int nbJoueurs) {
  ...
  int couleurs[8][3] = { {0,0,255}, {0, 255, 0}, {255, 0, 0}, {0, 255, 255}, {255, 255, 0}, {255, 0, 255}, {125, 125, 125}, {60, 0, 180}};
  ...
  for (i = 0; i < h; i++) {
    for (p = 0; p < w; p++) {
      ...
      for (j = 0; j < 30; j++) {
        ...
        if (ecart < (distanceMin - dist)) {
          ecart = distanceMin - dist;
        }
        if (dist < distanceMin) {
          distanceMin = dist;
